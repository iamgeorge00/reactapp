        ...
        editable={{
          onRowAdd:(newData)=>new Promise((resolve,reject)=>{
            const writeRow = async () => {
              ...
              await doc.loadInfo();   //load the documents info
              let sheet = doc.sheetsByIndex[2]; //Deductions    //index of the sheet
              const rows = await sheet.addRow({   //get all the rows
                'entry_date': newData.entry_date,
                'emp_eid': newData.emp_eid,
...
//                'remaining_balance' : parseInt(newData.opening_balance) - parseInt(newData.paid_balance),
            ...
            onRowUpdate: (newData,oldData) => new Promise((resolve,reject)=>{
              ...
              const updateRow = async () => {
                ...
                await doc.loadInfo();   //load the documents info
                let sheet = doc.sheetsByIndex[2]; //CODEntries    //index of the sheet
                const rows = await sheet.getRows();    //get all the rows
                ...
                rows[index].entry_date = newData?.entry_date;
                rows[index].emp_eid = newData?.emp_eid;
                rows[index].emp_name = newData?.emp_name;
                rows[index].amount = newData?.amount;
                rows[index].reason = newData?.reason;
